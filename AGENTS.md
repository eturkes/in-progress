...
- Network boundary: bind loopback. Mobile/remote access goes through private HTTPS reverse proxy (canonical: Tailscale Serve). Any broader bind requires explicit unsafe opt-in + documented risk.
- Commands: `pnpm check` = complete validation; `pnpm dev` = Vite + server; `pnpm build && pnpm start` = production.
- Runtime/generated state stays in `.data/` + `dist/`; fixtures stay hermetic in temp directories.
